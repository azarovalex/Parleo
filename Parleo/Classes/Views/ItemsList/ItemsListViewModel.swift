    ...
    func transform(input: Input) -> Output {
        ...
        input.selectedDriver
            .drive(onNext: { [weak self] indexPath in
                guard let self = self,
                    let item = self.items.value.first(where: { $0.text == self.cellViewModels.value[indexPath.row].title }) else {
                    return
                }
        ...
        input.deselectedDriver
            .drive(onNext: { [weak self] indexPath in
                guard let self = self,
                    let item = self.items.value.first(where: { $0.text == self.cellViewModels.value[indexPath.row].title }),
                    let index = self.selectedItems.firstIndex(where: { $0.text == item.text }) else {
                    return
                }
